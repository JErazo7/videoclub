...
$(document).ready(function () {
    ...
    editor.on( 'preSubmit', function ( e, o, action ) {
        if ( action !== 'remove' ) {
            var cedula = this.field( 'cedula' );
            var nombre = this.field( 'nombre' );
            var direccion = this.field( 'direccion' );
            var telefono = this.field( 'telefono' );

            if ( !cedula.val() ) {
                cedula.error('Debe ingresar un numero de cedula');
            }else if (cedula.val().length !=10 ) {
                cedula.error( 'Su numero de cedula debe tener 10 digitos' );
            }
            ...
            if ( !direccion.val() ) {
                direccion.error( 'Debe ingresar una direccion' );
            }

            if ( !telefono.val() ) {
                telefono.error( 'Debe ingresar una telefono' );
            }else if (telefono.val().length !=10 ) {
                telefono.error( 'Su numero de telefono debe tener 10 digitos' );
            }
